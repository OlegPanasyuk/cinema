...
class Film extends Component {
    ...
    render() {
        ...
        const { Poster, Ratings, imdbID, ...info } = film && (Object.keys(film).length > 0) && film;
        return (
            <>
            <Row>
                <Col>
                    <PageHeader onBack={this.redirectToBackward} title={`${info.Title} (${info.Year})`} subTitle={`${info.Production}`} />
                </Col>
            </Row>
            <Row>
                <Col span={20} offset={2}>
                    <Row gutter={{ xs: 0, sm: 16, md: 24 }}>
                        <Col sm={{ span: 8 }} xs={{ span: 24 }} lg={{ span: 8 }} md={{ span: 10 }} style={{ marginBottom: '25px'}}>
                            <img style={{ width: '100%', objectFit: 'cover'}} src={Poster} alt='Poster'/>
                        </Col>
                
                        <Col sm={{ span: 16 }} xs={{ span: 24 }} lg={{ span: 16 }} md={{ span: 14 }}>
                            
                            <Descriptions column ={{ xs: 1, sm: 1, md: 1}} bordered>
                            {
                                info && (Object.keys(info).length > 0) && Object.keys(info).map((el, i) => {
                                    return (
